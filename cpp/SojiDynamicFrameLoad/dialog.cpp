/*!
 * SojiDynamicFrameLoad
 * The demo demostrates how to interface to a Soji device via the REST API and WebSockets to upload a frame to Soji.
      ...
      _ui(new Ui::Dialog)
{
    ...
    connect(_ui->sendFramePushButton,SIGNAL (pressed()),this,SLOT (updateFrameToSoji()));
...
{
    QSettings settings(QSettings::UserScope, "aja" , "SojiDynamicFrameLoad");
...
void Dialog::updateFrameToSoji()
{

    QElapsedTimer timer;
    timer.start();

    // Setup Soji for Dynamic "Output" mode via REST and then send Frame. Frame Can either be a Image("FS02") or Overlay("FSA2")
    ...
    else
    {
        ...
        messageBox.setText("Soji Not Connected. Enter the IP Address of a Soji Device");
        messageBox.exec();
        ...
        else if ( _bitsPerComponent == 8  && _samplesPerPixel == 4)
        {
            ...
            for ( uint32_t lineCount=0; lineCount<_height; lineCount++)
            {
                ...
                for ( uint32_t pixelCount = 0; pixelCount < _width; pixelCount++)
                {
                    ...
                    // Note swapping red and blue to match Soji HW setup
                    *rgbaBuffer++ = blue;
                    *rgbaBuffer++ = green;
                    *rgbaBuffer++ = red;
                    *rgbaBuffer++ = alpha;
