/**
 * OpenAPI ColorBox
 * This is a REST API for the AJA ColorBox product.
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@aja.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 *       Do not edit the class manually.
 *
 * Copyright (C) 2020-2022 AJA Video Systems Inc.  All rights reserved.
 */

/*
 * OAIColorfrontConfig.h
 *
 * 
 */

#ifndef OAIColorfrontConfig_H
#define OAIColorfrontConfig_H

#include <QJsonObject>

#include "OAIColorfrontDynRangeGamut.h"
#include "OAIColorfrontLiveMode.h"
#include "OAIColorfrontMode.h"
#include "OAIColorfrontTvMode.h"

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIColorfrontConfig : public OAIObject {
public:
    OAIColorfrontConfig();
    OAIColorfrontConfig(QString json);
    ~OAIColorfrontConfig() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    bool isEnabled() const;
    void setEnabled(const bool &enabled);
    bool is_enabled_Set() const;
    bool is_enabled_Valid() const;

    OAIColorfrontDynRangeGamut getInDynRangeGamut() const;
    void setInDynRangeGamut(const OAIColorfrontDynRangeGamut &in_dyn_range_gamut);
    bool is_in_dyn_range_gamut_Set() const;
    bool is_in_dyn_range_gamut_Valid() const;

    OAIColorfrontDynRangeGamut getOutDynRangeGamut() const;
    void setOutDynRangeGamut(const OAIColorfrontDynRangeGamut &out_dyn_range_gamut);
    bool is_out_dyn_range_gamut_Set() const;
    bool is_out_dyn_range_gamut_Valid() const;

    OAIColorfrontMode getMode() const;
    void setMode(const OAIColorfrontMode &mode);
    bool is_mode_Set() const;
    bool is_mode_Valid() const;

    OAIColorfrontTvMode getTvMode() const;
    void setTvMode(const OAIColorfrontTvMode &tv_mode);
    bool is_tv_mode_Set() const;
    bool is_tv_mode_Valid() const;

    OAIColorfrontLiveMode getLiveMode() const;
    void setLiveMode(const OAIColorfrontLiveMode &live_mode);
    bool is_live_mode_Set() const;
    bool is_live_mode_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

    bool applyMinMaxConstraints();
private:
    void initializeModel();

    bool m_enabled;
    bool m_enabled_isSet;
    bool m_enabled_isValid;

    OAIColorfrontDynRangeGamut m_in_dyn_range_gamut;
    bool m_in_dyn_range_gamut_isSet;
    bool m_in_dyn_range_gamut_isValid;

    OAIColorfrontDynRangeGamut m_out_dyn_range_gamut;
    bool m_out_dyn_range_gamut_isSet;
    bool m_out_dyn_range_gamut_isValid;

    OAIColorfrontMode m_mode;
    bool m_mode_isSet;
    bool m_mode_isValid;

    OAIColorfrontTvMode m_tv_mode;
    bool m_tv_mode_isSet;
    bool m_tv_mode_isValid;

    OAIColorfrontLiveMode m_live_mode;
    bool m_live_mode_isSet;
    bool m_live_mode_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIColorfrontConfig)

#endif // OAIColorfrontConfig_H
