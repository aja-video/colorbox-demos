...
void OAIDefaultApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("getBuildInfo", defaultConf);
    _serverIndices.insert("getBuildInfo", 0);
...
void OAIDefaultApi::getBuildInfo() {
    QString fullPath = QString(_serverConfigs["getBuildInfo"][_serverIndices.value("getBuildInfo")].URL()+"/buildInfo");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIDefaultApi::getBuildInfoCallback);
    connect(this, &OAIDefaultApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            emit allPendingRequestsCompleted();
...
void OAIDefaultApi::getBuildInfoCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAIBuildInfo output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        emit getBuildInfoSignal(output);
        emit getBuildInfoSignalFull(worker, output);
    } else {
        emit getBuildInfoSignalE(output, error_type, error_str);
        emit getBuildInfoSignalEFull(worker, error_type, error_str);
    }
}
